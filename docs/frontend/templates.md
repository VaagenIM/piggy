...
Debug/example template that lists segment data, media header paths, links, and metadata. It is not referenced by the main route selection.
...
| `card_type` | Type string for future branching. |
| `show_*` flags | Toggle title, levels, difficulty, overlay, tags, description, thumbnail. |

The anchor receives `data-title`, `data-description`, `data-tags`, and `data-overlay-text`, which can support filtering/search-like card behavior.
...
### `templates/partials/parts/settings/dyslexia.html`

Old standalone dyslexia button partial. It is not included by current templates.
...
### `templates/partials/parts/breadcrumbs.html`

Compatibility wrapper that includes `partials/header/breadcrumbs.html`.
...
| `example.html` | Debug/example data listing. |
...
| `partials/parts/breadcrumbs.html` | Breadcrumb wrapper include. |
| `partials/parts/settings/search-button.html` | Search nav button. |
| `partials/parts/settings/dyslexia.html` | Legacy dyslexia button partial. |
