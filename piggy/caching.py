...
def _render_assignment(p: Path, extra_metadata=None) -> Response:
    ...
    all_metadata = dict(
        {
            **meta,
            **get_all_meta_from_path(str(p.parent), PIGGYMAP),
            **extra_metadata,
        }
    )
...
def _render_assignment_wildcard(path="", lang="") -> Response:
    ...
    Keeps track of template_type:
    assignments_root, year_level, class_name, subject, topic.

    Gets the relevant piggymap from the path.
    key = child content
    value = dict with relevant child data + meta
    """
    ...
    if len(path.split("/")) == AssignmentTemplate.ASSIGNMENT.index:
        ...
        # Render the assignment with the metadata.
        # Metadata must be deepfrozen to be hashable.
